			...
			new p5(p => {
				...
				p.mousePressed = () => {
				 ...
				 // If any of the active systems have this event, trigger it
					this.activeSystems.forEach(s => s.mousePressed?.(p, this.held))
				}
				p.mouseReleased = () => {
					 // If any of the active systems have this event, trigger it
				 this.activeSystems.forEach(s => s.mouseReleased?.(p, this.held))
				...
				p.mouseClicked = () => {
					// If any of the active systems have this event, trigger it
					this.activeSystems.forEach(s => s.mouseClicked?.(p))
				}
